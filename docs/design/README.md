...
|------|-------|--------|----------|
| 2026-07-24 | End of turn report — a **blocking, dismissable** modal shown right after **End Turn**, summarising how five planet metrics changed over the turn just elapsed (**Temperature · Emissions · CO₂ · Treasury · Biodiversity**) as `icon · label · value · colored Δ chip` (arrow = direction, color = good/bad, shared Turn-Log vocabulary; `— flat` when it rounds to zero). Reuses the `DataOverlay`/`EndingScreen` overlay pattern (Continue / ✕ / Esc / backdrop close). A 📊 reopen button sits in the `ResourceBar` (left of Money, hidden on turn 0). Pure `turnLog` diff — no engine change; no new tokens | **Implemented** | [proposal](./proposals/2026-07-24-end-of-turn-report/design.md) |
| 2026-07-09 | Viewport fit — the whole turn (resource bar, map, region info, **End Turn**, and **both full-size policy lanes**) fits one viewport with **no scroll**: the map has a **viewport-derived fixed height** `clamp(180px, calc(100dvh − 376px), 560px)` so it's the **same size with or without a region selected** (no shrink on click); the two policy lanes go **side-by-side** (Active \| Available) instead of stacked, which keeps the board one card-row tall and leaves the map ~430px; **End Turn** is an **icon button (⏭, tooltip)** in the board's right gutter; `RegionInfoBox` compacted. Cards untouched; Turn Log below the fold. No new tokens | **Implemented** | [proposal](./proposals/2026-07-09-viewport-fit-no-scroll/design.md) |
| 2026-07-08 | Electricity panel — trim the first heading to just **Generation mix** (drop the `— share of power · = 100%` note) and add a right-aligned **total power generation** figure (real **TWh/yr**, derived from `electricityDemand × twhPerDemandUnit` so it scales with demand growth; planet = Σ regions). No new tokens | **Implemented** | [proposal](./proposals/2026-07-08-electricity-total-generation/design.md) |
| 2026-07-08 | Electricity infographic — the drill-down's **Electricity** node becomes two **separated** graphics: a **generation-mix donut** (8 sources, `Fossil` vs `Clean` grouped legend, clean-share in the hole) over **converging emission streams** (stream width = a source's Gt, merging to the electricity total; nuclear + renewables are dashed, zero-width, labelled `0`). Adds a neutral **`— flat`** change chip. Makes the falling coal *generation* share legible even while coal *emissions* lag behind demand growth | **Implemented** | [proposal](./proposals/2026-07-08-electricity-infographic/design.md) |
