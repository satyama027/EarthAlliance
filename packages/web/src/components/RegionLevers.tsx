...
/**
 * The four coupling variables the policies move, shown as a 2×2 grid with hover tooltips. Takes the
 * primitive lever values so both the region panel (per-region) and the planet panel (aggregated) can
 * feed it.
 */
export function RegionLevers({ gridIntensity, storage, cropYield, powerDemand }: {
  gridIntensity: number; storage: number; cropYield: number; powerDemand: number;
}) {
  return (
    <Box style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px 14px' }}>
      <Lever
        ...
        value={gridIntensity.toFixed(2)}
        barPct={gridIntensity * 100}
